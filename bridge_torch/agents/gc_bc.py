...
class GCBCPolicy(nn.Module):
    def __init__(
        ...
        early_goal_concat: bool = True,
        use_proprio: bool = False,
    ):
        super().__init__()
        self.obs_encoder = obs_encoder
        # 当共享编码器时，不注册第二个模块，避免 torch.compile 对同一模块的双引用冲突
    ...
    def forward(self, obs: Dict[str, torch.Tensor], goal: Dict[str, torch.Tensor]):
        xo = obs["image"]  # (N, C, H, W)
        xg = goal["image"]
        zo = self.obs_encoder(xo)
        # 安全获取目标编码器；未提供时复用 obs_encoder
        ge = getattr(self, "goal_encoder", None)
        zg = self.obs_encoder(xg) if ge is None else ge(xg)
        if self.early:
            z = torch.cat([zo, zg], dim=-1)
        else:
            z = zo + zg
...
class GCBCAgent:
    def __init__(self, model: GCBCPolicy, cfg: GCBCConfig, action_dim: int, device: torch.device):
    ...
    def update(self, batch: Dict[str, torch.Tensor]):
        ...
        goal = {k: v.to(self.device, non_blocking=True) for k, v in batch["goals"].items() if isinstance(v, torch.Tensor)}
    ...
    @torch.no_grad()
    def sample_actions(self, obs: Dict[str, torch.Tensor], goal: Dict[str, torch.Tensor], argmax: bool = False):
        self.model.eval()
        out = self.model(
            {k: v.to(self.device, non_blocking=True) for k, v in obs.items()},
            {k: v.to(self.device, non_blocking=True) for k, v in goal.items()},
        )
        dist = self.model.dist(out)
        if argmax:
            return out["mu"].cpu()
